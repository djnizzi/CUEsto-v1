...
export const DiscogsModal: React.FC<DiscogsModalProps> = ({ isOpen, onClose, onSuccess, totalDuration }) => {
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            if (response?.result) {
                ...
                setError(response?.error || 'Failed to fetch release from Discogs.');
            }
        } catch (e: any) {
            setError('Connection with Discogs failed. Please retry.');
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm px-4">
            ...
            >
                ...
                <h2 className="text-brand-text font-semibold text-[15px] leading-tight">
                    import metadata from Discogs
                </h2>

                {/* Help Text */}
                <p className="text-[#A8A29E] text-[13px] font-light leading-relaxed">
                    search release from their web page{' '}
                    <a
                    ...
                    , and copy the release code from a release page (master releases are not valid). it is possible to interpolate start times and durations from a release, if it has track durations, with your file's total timing. the resulting durations are estimates and not exact, as Discogs timings don't have the precision of those in a .cue file
                </p>
                ...
                <div className="flex flex-col gap-4 mb-2">
                    ...
                    />
                    <CheckboxItem
                        label="interpolate start times/durations"
